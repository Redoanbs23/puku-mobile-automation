...
3. **Reconciled against the current GitHub-side state** — confirmed the QA2 GitHub account is currently under abuse-detection review, which prevents the GitHub-side operations needed to make Stage 2 operational: pushing/merging branches/PRs, manually triggering workflows, configuring repository secrets, configuring the `ci-pre-authenticated-snapshot` Environment, and configuring branch protection. This is an external blocker, not a missing in-repo change.
...
- **The current QA2 GitHub abuse-detection review blocks validation and configuration work from the normal QA channel.** Until that review resolves, no PRs can trigger the CI checks and hence cannot be merged, and no repository secrets / Environments / branch protection can/should be configured. This affects all work that depends on GitHub-side validation, not just Stage 2.
...
- **The six already-implemented-but-unmerged branches (`LOGIN-E2E-009`/`010`/`011`/`012`, `CHAT-E2E-008`/`019`) are an integration / PR-access problem, not an authorship gap.** Their implementations exist on dedicated branches; making them part of CI requires the same GitHub-side operations (push, PR, review, merge) that the QA2 abuse-detection review currently blocks. No further authorship is needed on any of them for CI-readiness purposes.
