...
## 2026-08-27 (QA2 — CHAT-E2E-008 controlled R8 execution)

### Session Summary

Finalized the controlled CHAT-E2E-008 R8 experiment. Corrected an invalid pre-send precondition after live execution surfaced a DOM-class mismatch between pre-send and post-send user-bubble shapes. Executed the ONE controlled R8 run on the physical Samsung Galaxy A13 (`R58T90F5ALY`); the run reached a real PUKU `DioException`/`HttpException: Software caused connection abort` recoverable error surface but the automated test technically FAILED because `diagnose()` constructed an invalid `RegExp` from `TEST_MESSAGE`'s square brackets. Captured `post-cut-1s.xml` (only) before the diagnostic exception aborted the remaining checkpoints. Wi-Fi restoration and drawer/new-chat cleanup both ran. Diagnosed and fixed the harness bug (substring match instead of `new RegExp(...)`). Static validation after the correction: typecheck PASS, lint PASS. **R8 budget is exhausted** — the shared test account `editorpuku@gmail.com` consumed its allocated R8 send during this run; no rerun will occur.

### Chronological Log

1. **Reviewed the controlled CHAT-E2E-008 R8 experiment** as left from the morning session: the deterministic, test-thread spec at `tests/specs/chat/network-drop.spec.ts` (the previously plumbing-only spec, replaced earlier today with a send + Wi-Fi-cut-back-to-back implementation), the supporting `setWifiEnabled(enabled)` export from `src/utils/adb.ts`, and the matching Status update in `test-cases/chat/CHAT-TC-008.md`. All three preconditions documented in the spec — `mInputShown=true`, exact typed text on the EditText, and a "user bubble rendered before the cut" — were inspected against the existing recon evidence (`chat008-pre-send.xml`, `chat008-sent.xml`).

2. **Live execution revealed a flawed pre-send user-bubble precondition.** The first controlled run failed at `tests/specs/chat/network-drop.spec.ts:216` with `Expect //android.view.View[@text="[PUKU-QA-TEST:CHAT-TC-008] Say hello in one word."] to be displayed — Received: not displayed`. **Root cause:** the typed text pre-send lives on an `android.widget.EditText` (Flutter's custom-rendered EditText), while the rendered user bubble post-send is a separate `android.view.View`. `chat008-pre-send.xml` (typed text present, send not yet tapped) shows the text only on the EditText at bounds `[93,1082][920,1195]` — no View on the page has `text="[PUKU-QA-TEST:CHAT-TC-008] ..."` pre-send. `chat008-sent.xml` (post-send) shows the same text on a new View at bounds `[45,329][1035,543]`. The XPath `//android.view.View[@text="${TEST_MESSAGE}"]` matches a class (`android.view.View`) that does not exist pre-send — the assertion is structurally unreachable, not a timing problem.

3. **Applied the minimal correction**: removed the pre-send user-bubble assertion and its preceding comment block; the final pre-send verification on the test side is now `homeScreen.chatInputFieldWithText.getText() === TEST_MESSAGE` (which already passes). Renumbered the in-body step comments (`7→6`, `8→7`, `9→8`, `10→9`, `11→10`). The load-bearing critical-timing block — `await homeScreen.tapSendButton(); setWifiEnabled(false);` with NOTHING between — remained byte-identical. Updated the JSDoc precondition list (dropped "user bubble" item #3, added an explanatory NOTE block citing `chat008-pre-send.xml` vs `chat008-sent.xml` and the EditText vs View DOM-class mismatch). Updated the CHAT-TC-008.md Status section implementation-step list from 9 items to 8 items (dropped step 4, renumbered). Reconciled the false claim "user bubble rendered in the semantics tree BEFORE the cut was dispatched" with the actual pre/post-send DOM evidence. **No source/test change beyond those documented in the corrected diff.** Typecheck and lint both PASS.

5. **Executed the ONE controlled R8 run** (per the explicit instruction "this execution consumes the one remaining R8 budget allocated to CHAT-E2E-008"). Command: `DEVICE_UDID=R58T90F5ALY npm test -- --mochaOpts.grep="CHAT-E2E-008"`. The first invocation under PowerShell syntax `$env:DEVICE_UDID=...` SKIPPED because the bash sandbox used by the assistant does not parse PowerShell environment-variable syntax — the test's `if (!process.env.DEVICE_UDID) { this.skip(); }` guard fired correctly and zero AI sends occurred. The second invocation under POSIX prefix form `DEVICE_UDID=R58T90F5ALY npm test ...` ran the spec end to end.

6. **The run reached a real PUKU network-error surface.** `driver.getPageSource()` at the post-cut-1s checkpoint captured a Flutter-rendered error panel in the message-list area at bounds `[0,1972][1080,2273]` with `content-desc="Exception: DioException [unknown]: null\nError: HttpException: Software caused connection abort, uri = https://chat.api.puku.sh/v1/chat/conversations"`, `scrollable="true"`, `live-region="1"`, `dismissable="true"`. The chat composer (`chatInputFieldWithText`) was rendered at `[93,2011][987,2068]` with `text=""` and `hint="Chat with Puku..."` — the user message had been consumed and the composer reset for a retry. The model chip (ImageView at `[194,2118][477,2175]`) showed `content-desc="puku-ai-2.7"`. Package was `sh.puku.app` (foreground, no system overlay). The send-tap and Wi-Fi cut executed back-to-back as designed; no `driver.pause` between them.

7. **The automated test technically FAILED** because `diagnose()` (a diagnostic logger, never asserted on) constructed a `new RegExp(\`text="${TEST_MESSAGE}"\`)` and TEST_MESSAGE contains `[PUKU-QA-TEST:CHAT-TC-008]` — square brackets. JS's `RegExp` constructor threw synchronously: `SyntaxError: Invalid regular expression: /text="[PUKU-QA-TEST:CHAT-TC-008] Say hello in one word."/: Range out of order in character class` at `tests/specs/chat/network-drop.spec.ts:170`. The exception aborted the post-cut loop after the first iteration's XML was persisted; `post-cut-3s.xml`, `post-cut-8s.xml`, and `post-cut-20s.xml` were never written. **No `[008 post-cut-*]` diagnostic line was emitted** because `diagnose()` threw before `console.log` ran. The test's hard no-crash assertions (foreground package + composer displayed) were never reached.

8. **This is a test-harness diagnostic bug, NOT an observed PUKU application crash.** The `DioException` panel captured in `post-cut-1s.xml` is a real, in-app recoverable error surface — Flutter delivered the `HttpException` from PUKU's chat backend to the user via a `live-region=1` accessibility announcement. The app did NOT crash, did NOT hand off to a system overlay, did NOT hang. The conversation list contained an empty conversation (no user bubble, no AI response bubble rendered) — the send-tap dispatched a request that the cut interrupted before any AI response arrived, but the message itself was also not visibly rendered (the user message is sent and consumed without a bubble in this code path; the error panel replaces it).

9. **Cleanup ran.** The `finally` block executed: `setWifiEnabled(true)` restored Wi-Fi; `settingsScreen.tapHamburgerMenuTrigger()` + `drawerScreen.newChatButton.click()` performed the drawer-based clean/default-state reset. Post-run device state: `adb -s R58T90F5ALY shell settings get global wifi_on` → `1` (Wi-Fi ON); `cmd connectivity airplane-mode` → `disabled`. Failure-capture artifacts pulled: `test-results/failures/chat-e2e-008-p1-cuts-wi-fi-programmatically-immediately-after-send/recording.mp4` (~4 MB), `screenshot.png`, `logcat.txt`.

10. **Diagnosed and fixed the `diagnose()` bug.** The harness's substring filter was using `new RegExp(\`text="${TEST_MESSAGE}"\`)` to check whether each captured `text="..."` attribute equals the user-bubble text. Replacing with a plain `===` substring comparison against `\`text="${TEST_MESSAGE}"\`` (a string, not a regex) gives identical filter semantics for this use case and avoids the regex character-class parsing entirely. Diff: a single variable rename (`userBubbleRe` → `userBubbleSubstr`) and a method swap (`.test(m)` → `m !== userBubbleSubstr`), with a 7-line comment block citing the 2026-08-27 live failure. No other changes to `network-drop.spec.ts` (the send/cut block, the post-cut checkpoints, the hard no-crash assertions, the `finally` cleanup, the R8 GATE — all byte-identical).

11. **Static validation after the correction**: `npx tsc --noEmit -p tsconfig.json` → PASS (exit 0, no output). `npm run lint` → PASS (exit 0, no output). `post-cut-1s.xml` byte-identical to the post-run state (Modify time still `2026-08-27 20:04:20`, Size still `11,230 bytes`).

### Key Decisions

- **The pre-send user-bubble assertion was structurally unreachable** and was removed rather than re-scoped. The corrected spec's final pre-send verification is the existing-class-only EditText text check, which matches how every established chat-core spec (CHAT-E2E-002/007/014/017) implicitly establishes its pre-send state.
- **The R8 budget for CHAT-E2E-008 is exhausted.** The single controlled run consumed one AI send on `editorpuku@gmail.com`. The captured `post-cut-1s.xml` is the evidence; no second run will be attempted.
- **The graceful-error contract is supported by direct captured evidence** (`DioException [unknown]` + `HttpException: Software caused connection abort` panel at `[0,1972][1080,2273]`, `live-region=1`, `dismissable=true`). It is NOT asserted by the automated test, because the harness aborted before reaching the hard no-crash assertions. A future implementer can derive a real `homeScreen.networkErrorSurface` locator from the captured `content-desc` once the diagnostic-logging bug is fixed and a no-bug rerun captures all four post-cut XMLs.
- **Priority remains P1 — pending a clean automated rerun.** The morning entry's "pending evidence" framing is now resolved in the direction of P1 (graceful error observed, no crash), but the automated test still has not reached its hard assertions in a green run.

### Observations / Doc Gaps

- **`diagnose()`'s `new RegExp(\`text="${TEST_MESSAGE}"\`)` was an unnecessary regex use in the first place** — `.includes(\`text="${TEST_MESSAGE}"\`)` (the line above it) already answers the same yes/no question without any regex. The filter's per-match boolean was the only consumer that needed regex semantics, and even that can be answered with plain `===` against a string template. Lesson: do not promote a substring check to a `new RegExp(...)` unless the substring actually contains regex metacharacters that would be invalid in a regex (and even then, prefer `String.prototype.includes` over a RegExp when no pattern semantics are needed). Recorded for `ai-log/lessons-learned.md` as a follow-up.
- **`chat008-pre-send.xml` vs `chat008-sent.xml` are now load-bearing evidence** for the pre/post-send EditText/View DOM-class mismatch. They should not be deleted from `evidence/008-recon-2026-08-27/` without preserving the bounds citations in CHAT-TC-008.md's Status section.
- **The four-checkpoint post-cut observation loop remains valuable even though only the first iteration completed** — the `DioException` panel in `post-cut-1s.xml` confirms the cut landed in time (no response bubble at +1 s); subsequent checkpoints in a no-bug rerun would confirm whether the panel persists, transforms into a retry affordance, or is dismissed by the user. The fix unblocks a future clean run that needs no additional R8 budget allocation.
- **`test-results/008-2026-08-27/post-cut-1s.xml` is the only evidence file produced by this run.** Files for `post-cut-3s`, `post-cut-8s`, and `post-cut-20s` were never written because the diagnostic exception aborted the loop after iteration 1. The failure-capture hook did pull `/sdcard/test-failure.mp4` and a screenshot to `test-results/failures/chat-e2e-008-p1-cuts-wi-fi-programmatically-immediately-after-send/`; those are the run's video evidence.

### Next Steps

1. **Do NOT rerun CHAT-E2E-008.** The R8 budget for this experiment is exhausted; the captured `post-cut-1s.xml` is the technical evidence; the recorded ground truth (user-observed) confirms the conversation list was left empty.
2. **Update CHAT-TC-008.md Status section** to reflect today's final outcome — controlled experiment executed once, R8 budget consumed, test technically failed due to harness `diagnose()` bug, `post-cut-1s.xml` captured a real DioException/HttpException surface, app did not crash, Wi-Fi restored, cleanup ran. The priority remains P1 (pending a clean automated rerun once a future R8 allocation is granted).
3. **Recorded harness bug** for `ai-log/lessons-learned.md` follow-up: do not use `new RegExp(substring)` when substring is a literal; prefer `String.prototype.includes` or `===` against a string template. (Not yet added to lessons-learned.md — separate doc update, not this session's scope.)
4. **Carry-forward, unchanged**: R13 exploration pass, R10 chat-deletion investigation, the `00-apk-reconnaissance.md`/ADR-numbering/`env.ts` cleanup items from 2026-08-06, CHAT-E2E-017's still-blocked rotation+keyboard state.
---
